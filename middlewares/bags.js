...
const sItems = require('../models/bags');

module.exports = {
    getDataBase:  function(req, res){
        request('https://api.mercadolibre.com/sites/MLM/search?q=bags#json', {json:true}, (err, response, body)=>{
            if(err){
                console.log(err);
            }
            var items = [];
            ...
            res.send({message: 'elementos guardados en la base de datos'});
